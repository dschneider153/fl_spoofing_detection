...
def synthesize_spoofing_labels(df):
    ...
    df["spoofing_score"] += (
        (df["distance_ticks"] > 5) & (df["distance_ticks"] < 50)
    ).astype(float) * 2
    df["spoofing_score"] += (
        (df["distance_ticks"] > 10) & (df["distance_ticks"] < 50)
    ).astype(float) * 1
...
print(
    features_df[features_df["spoofing_score"] > 0.9][
        [
            "relative_size",
            "distance_ticks",
            "log_lifetime",
...
print(
    features_df[features_df["spoofing_score"] < 0.3][
        [
            "relative_size",
            "distance_ticks",
            "log_lifetime",
