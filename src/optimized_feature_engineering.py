...
if (mbo["price"].sort_values().diff().dropna().loc[lambda x: x > 0].min()) <= 0:
    TICK_SIZE = 0.005
else:
...
anchor_events_with_mbp["midprice_change_during"] = vectorized_midprice(
    t_end_arr
) - vectorized_midprice(t_add_arr)
anchor_events_with_mbp["midprice_change_50ms"] = vectorized_midprice(
    t_end_arr + ms50
) - vectorized_midprice(t_end_arr)
anchor_events_with_mbp["midprice_change_200ms"] = vectorized_midprice(
    t_end_arr + ms200
) - vectorized_midprice(t_end_arr)
anchor_events_with_mbp["midprice_change_1000ms"] = vectorized_midprice(
    t_end_arr + ms1000
) - vectorized_midprice(t_end_arr)
...
anchor_events_with_mbp["spread_change_ticks"] = (
    np.abs((vectorized_spread(t_end_arr + POST_MS_np) - vectorized_spread(t_end_arr)))
    / TICK_SIZE
)
# Imbalance lookup
...
for side in ["B", "A", "N"]:
    ...
    side_adds["rolling_baseline"] = (
        side_adds["size"].rolling("5s", closed="left").mean().fillna(1.0)
    )
